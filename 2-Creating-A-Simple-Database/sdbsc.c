...
int get_student(int fd, int id, student_t *s){
    ...
    off_t offset = STUDENT_RECORD_SIZE * (id - MIN_STD_ID);
...
int add_student(int fd, int id, char *fname, char *lname, int gpa){
    ...
    student_t s;
    int student = get_student(fd, id, &s);
    ...
    off_t offset = STUDENT_RECORD_SIZE * (id - MIN_STD_ID);
    if (lseek(fd, offset, SEEK_SET) == -1 || write(fd, &newStudent, STUDENT_RECORD_SIZE) != STUDENT_RECORD_SIZE) {
...
int del_student(int fd, int id){
    ...
    if (lseek(fd, offset, SEEK_SET) == -1 || write(fd, &empty, STUDENT_RECORD_SIZE) != STUDENT_RECORD_SIZE) {
        fprintf(stderr, M_ERR_DB_WRITE);
        return ERR_DB_FILE;
    }
...
void print_student(student_t *s){
    ...
    if (s == NULL || s->id < MIN_STD_ID || s->id > MAX_STD_ID) {
